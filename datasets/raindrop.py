...
class RainDrop:
    ...
    def get_loaders(self, parse_patches=True, validation='raindrop'):
        ...
        train_dataset = RainDropDataset(dir=os.path.join(self.config.data.data_dir, 'raindrop', 'train'),
                                        n=self.config.training.patch_n,
                                        patch_size=self.config.data.patch_size,
                                        transforms=self.transforms,
                                        filelist=None,
                                        parse_patches=parse_patches)
        val_dataset = RainDropDataset(dir=os.path.join(self.config.data.data_dir, 'raindrop', 'raindrop_test'),
                                      n=self.config.training.patch_n,
                                      patch_size=self.config.data.patch_size,
                                      transforms=self.transforms,
...
class RainDropDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        ...
        if filelist is None:
            ...
            raindrop_inputs = os.path.join(raindrop_dir, 'input')
            ...
            gt_names += [os.path.join(os.path.join(raindrop_dir, 'gt'), i.replace('rain', 'clean')) for i in images]
