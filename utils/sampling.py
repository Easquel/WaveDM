...
def generalized_steps_overlapping(x, x_cond, seq, model, b, eta=0., corners=None, p_size=None, manual_batching=True,total=None,use_global=False,use_FFT=False):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        x0_preds = []
        ...
        for i_t, j_t in zip(reversed(seq), reversed(seq_next)):
            ...
            if manual_batching:
                ...
                for i in range(0, len(corners), manual_batching_size):
                    batch_x_cond = x_cond_patch[i:i + manual_batching_size]
                    if use_FFT:
                        x_fft = torch.fft.fft2(batch_x_cond, dim=(-2, -1))
                        x_amp = torch.abs(x_fft)
                        x_phase = torch.angle(x_fft)
                        batch_x_cond = torch.cat([batch_x_cond, x_amp, x_phase], dim=1)
                    if use_global == False:
                        x_cond_and_t_patch = torch.cat([batch_x_cond, xt_patch[i:i + manual_batching_size]], dim=1)
            ...
            xt_next = at_next.sqrt() * x0_t + c1 * torch.randn_like(x) + c2 * et
            xs.append(xt_next.to('cpu'))
            print(f"t:{i_t} e pred mean:{et.mean().item()} e pred std:{et.std().item()} x0 pred:{x0_t.mean().item()} x next:{xt_next.mean().item()}")
